            ...
            if proxy_enabled and not same_domain:
                if check_query(resource):
                    if oauth_req == 'true' and not p.toolkit.c.user:
                        details = "</br></br><H3>In order to see this resource properly, you need to be logged in</H3></br></br></br>"
                        h.flash_error(details, allow_html=False)
                        view_enable = [False, details]
                        url = proxy.get_proxified_resource_url(data_dict)

                    elif oauth_req == 'true' and not oauth2_enabled:
                        details = "</br></br><H3>In order to see this resource properly, enable oauth2 extension</H3></br></br></br>"
                        h.flash_error(details, allow_html=False)
                        view_enable = [False, details]
                        url = proxy.get_proxified_resource_url(data_dict)
                    ...
                    details = "</br></br><H3>This is not a ContextBroker query, please check <a href='https://forge.fiware.org/plugins/mediawiki/wiki/fiware/index.php/Publish/Subscribe_Broker_-_Orion_Context_Broker_-_User_and_Programmers_Guide'>Orion Context Broker documentation</a></H3></br></br></br>"
                    h.flash_error(details, allow_html=False)
                    view_enable = [False, details]
                    url = proxy.get_proxified_resource_url(data_dict)
            else:
                if proxy_enabled:
                    details = "</br></br><H3><Enable resource_proxy"
                else:
                    details = "</br></br><H3>This is not a ContextBroker query, please check <a href='https://forge.fiware.org/plugins/mediawiki/wiki/fiware/index.php/Publish/Subscribe_Broker_-_Orion_Context_Broker_-_User_and_Programmers_Guide'>Orion Context Broker documentation</a></H3></br></br></br>"
